...
/**
 * A visible, genuine scripted **failure** path — real terminal-failure
 * evidence from the real controller, not an invented envelope. Selected by
 * exact input text, so the UI can offer it as a labelled, deliberate action
 * ("Send a message that fails") rather than an unpredictable trigger phrase.
 */
export const SCRIPTED_FAILURE_TRIGGER_TEXT = 'trigger scripted failure';

const FAILURE_SCRIPT: readonly ScriptStep[] = [
  { kind: 'diagnostic', level: 'info', message: 'scripted demo — about to fail deliberately' },
  { kind: 'delta', text: 'Scripted demo provider is about to demonstrate a failure path.' },
  { kind: 'fail', message: 'scripted demo: deliberate failure requested by input text' },
];

/**
 * A long, purely mechanical burst — used only to exercise this app's bounded
 * SSE transport and the protocol's subscriber-overflow signal under a real
 * HTTP connection (see `test/reference-app-reconnect.test.ts`). Selected by
 * exact input text like the other scripts; not surfaced anywhere in the UI.
 */
export const SCRIPTED_BURST_TRIGGER_TEXT = 'trigger scripted burst (test only)';
const BURST_STEP_COUNT = 400;
const BURST_SCRIPT: readonly ScriptStep[] = [
  ...Array.from({ length: BURST_STEP_COUNT }, (_, index): ScriptStep => ({
    kind: 'delta',
    text: `burst delta ${String(index).padStart(6, '0')} `.repeat(4),
  })),
  { kind: 'succeed' },
];
...
export function createScriptedDemoProvider(): ScriptedDemoProvider {
  const { provider, controller } = createScriptedProvider({
    ...
    scripts: {
      [SCRIPTED_FAILURE_TRIGGER_TEXT]: FAILURE_SCRIPT,
      [SCRIPTED_BURST_TRIGGER_TEXT]: BURST_SCRIPT,
    },
