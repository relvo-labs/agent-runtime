...
One Runtime instance coordinates active work first by command ID and then by session. This makes the receipt check and effect atomic in-process, admits at most one competing interaction response, and does not serialize unrelated sessions. Entries exist only while work or waiters remain, except a failed external effect retains its fingerprint until exact retry or shutdown resolves its cleanup. Multiple Runtime processes require a host-provided single-writer boundary and durable transactional command claim before provider side effects; the in-memory coordination and attempt maps are not distributed locking.
