...
This document names the **foundation milestone**, not the wire minor. The wire line it
describes has since moved to `0.5`; the filename is stable so the skill ownership map and
every inbound link stay valid. Where a wire fact appears below it states the current line,
and `docs/adr/` records each move.
...
Question, multi-question and approval requests and responses are separately discriminated. Responses must match request kind, permitted choices, selection cardinality, and approval mode. A `question_set` request carries an ordered list of keyed questions, each with its own prompt, optional header, optional choice list, selection mode, free-text affordance and sensitivity flag; its response is a record keyed by those question keys. Settlement is all-or-nothing: the answered key set must equal the asked key set exactly, with no missing question and no unknown key, before any provider is touched, so no adapter can receive a partially answered batch. Question keys are adapter-assigned and never provider-native. Key uniqueness within a request is enforced in Zod and at in-process ingress; Draft 2020-12 has no keyword for uniqueness by a property across array items, and that representability boundary is asserted explicitly in the Ajv parity corpus rather than left implicit. A pending interaction is settled once as responded, cancelled, expired, or withdrawn. A provider withdraws a request it raised by emitting `interaction.withdrawn` with the reference it was given; the runtime stamps the settlement, clears routing, and resumes the run, and ignores a withdrawal for an interaction whose response is already a retained logical settlement. Response commands and provider completion finalization share the per-session transition coordinator and re-check pending state inside the commit, so a completion race cannot append a second settlement. A response already delivered to the provider remains a retained logical settlement if its first store commit fails; completion materializes that response before validating the terminal outcome, and exact retry records the original applied result without redelivery. Otherwise provider termination is validated against the pre-settlement run state; cancelling an awaiting interaction cannot retroactively make provider success legal. A repeated identical command is handled by command idempotency; a new command targeting a settled interaction is stale and rejected.
